...
TOKEN = '6211543841:AAHRbmEXDjWNwtMaC5EQNUY5I205dyOEP8I'
  ...
  if callback.data.startswith('delete_'):
    ...
    if delete_data(name_to_delete):
      bot.send_message(callback.message.chat.id,
                       f'Товар "{name_to_delete}" успешно удален.')
      bot.delete_message(callback.message.chat.id, callback.message.message_id)
    else:
      bot.send_message(callback.message.chat.id,
                       f'Ошибка при удалении товара "{name_to_delete}".')
    ...
    if product:
      ...
      print(product_data)
      post(product_data[0], product_data[1], product_data[2], product_data[3],
           photopath)
      #sleep(10000)
      ...
      bot.send_message(callback.message.chat.id,
                       f'Listing {product_data[0]} succesful listed')
    else:
      bot.send_message(callback.message.chat.id,
                       f'Error publishing product: "{product_name}".')
...
@bot.message_handler(commands=[
    'info', 'site', 'help', 'id', 'Hi', 'start', 'hello', 'Создать новое',
    'read', 'Привет'
])
def on_start(message, markup):
  if message.text.startswith('/'):
    if message.text == '/info':
      ...
      webbrowser.open(
          url='https://www.youtube.com/channel/UCW4OwywfIvGvyrGmV3TWDgA')
    ...
    if message.text == 'Создать новое':
      ...
      bot.send_message(message.chat.id,
                       'Привет  хочешь узнать что я умею? \n/help',
                       reply_markup=markup)
...
def save_price(message, name):
  ...
  if not price:
    ...
    save_category(message, name, price)
...
def save_description(message, name, price):
  ...
  bot.send_message(message.chat.id,
                   f'Описание товара сохранено: {description}')
  markup = types.ReplyKeyboardMarkup()
  bt1 = types.KeyboardButton('Новый товар')
  bt2 = types.KeyboardButton('Б/у в идельном состоянии')
  ...
  bot.register_next_step_handler(message, condition_select, name, price,
                                 description)
...
def condition_select(message, name, price, description):
  if message.text == 'Новый товар':
  ...
  elif message.text == 'Б/у в идельном состоянии':
    condition = 'Б/у в идельном состоянии'
    send_photos(message, name, price, description, condition)
    ...
    print(f"DEBUG: save: eror: no correct condition: name: {name}")
    markup = types.InlineKeyboardMarkup()
    bt1 = types.InlineKeyboardButton('Создать новое', callback_data='new')
    markup.row(bt1)
    bot.send_message(
        message.chat.id,
        'Обьявление не создано так как состояние нужно выбирать из кнопок на клавиатуре ⌨️',
        reply_markup=markup)
...
def send_photos(message, name, price, description, condition):
  ...
  bot.register_next_step_handler(message, save_photos, name, price, condition,
                                 description)
...
def save_photos(message, name, price, condition, description, photos=[]):
  if message.content_type == 'photo':
    ...
    bot.register_next_step_handler(message, save_photos, name, price,
    ...
    bot.send_message(message.chat.id,
                     'Ошибка! Отправьте фотографию или напишите /done.')
